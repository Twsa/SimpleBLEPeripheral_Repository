...
#define TEST_LED_MODE_ALL_BLINK   0  //ËùÓÐÉÁ¶¯
#define TEST_LED_MODE_ALL_FLOW    1  //ËùÓÐÁ÷¶¯ÉÁ¶¯
#define TEST_LED_MODE_KEY_CTRL    2  //°´¼ü¿ØÖÆµÈ
#define TEST_LED_MODE_HOST_CTRL   3  //Ö÷»úÁ¬½ÓºóÖ»ÓÉÖ÷»ú¿ØÖÆ
...
//void  UartCallBack(uint16_t rxLen, uint16_t txLen)
//{
//    if(rxLen > 0)//´®¿Ú½ÓÊÕµ½Êý¾ÝÁË
//    {   // ±£´æÊý¾Ýµ½¶ÓÁÐÖÐ
//        qq_write(UART_GetRxBufferAddress(), rxLen);
//        // ·¢ËÍÒ»¸öÍ¨ÖªÏûÏ¢ SBP_UART_CHANGE_EVT£¬ ÒÔ±ã°ÑÊý¾Ý·¢ËÍµ½¿Õ¿Ú
...
static void SimpleBLEPeripheral_init(void)
{
  ...
  Board_initKeys(SimpleBLEPeripheral_keyChangeHandler);          //°´¼ü³õÊ¼»¯

  // led³õÊ¼»¯
  HalLedInit();                                                  //led³õÊ¼»¯
  Uart_Init(UartCallBack);// ´®¿ÚÇý¶¯Ìí¼Ó£¬ ÊÕµ½´®¿ÚÊý¾ÝÊ±½«Ö´ÐÐ UartCallBack º¯Êý
...
#endif // FEATURE_OAD
  LCD_WRITE_STRING("WWW.AMOMCU.COM", LCD_PAGE7);
}
uint16_t OneShotADC_value;
/*********************************************************************
...
static void SimpleBLEPeripheral_taskFxn(UArg a0, UArg a1)
{
  ...
  SimpleBLEPeripheral_init();                                   //³õÊ¼»¯Ö»°üÀ¨ÁËÐèÒªµÈ´ýµÄ°´¼üÏûÏ¢ÔÚÏûÏ¢¶ÓÁÐÖÐ                     
  ...
  for (;;)
  {
    ...
    ICall_Errno errno = ICall_wait(ICALL_TIMEOUT_FOREVER);       //µÈ´ýÈÎÎñÏûÏ¢ÈëÁÐµÄÐÅºÅ  
    ...
    {   
      ...
      // If RTOS queue is not empty, process app message.           //Èç¹ûÏûÏ¢¶ÓÁÐ²»Îª¿ÕÔò½øÐÐ½ø³ÌµÄÏûÏ¢´¦Àí
      while (!Queue_empty(appMsgQueue))
      {
        sbpEvt_t *pMsg = (sbpEvt_t *)Util_dequeueMsg(appMsgQueue);  //Í¨ÓÃÏûÏ¢³öÁÐ£¬»ñÈ¡ÊÂ¼þÏûÏ¢
        if (pMsg)
        {
          // Process message.
          SimpleBLEPeripheral_processAppMsg(pMsg);                  //Ö´ÐÐÏàÓ¦µÄÏûÏ¢¶ÔÓ¦µÄÈÎÎñ£¬Ö±ÖÁÏûÏ¢¶ÓÁÐÎª¿Õ

          // Free the space from the message.
          ICall_free(pMsg);                                         //ÊÍ·ÅÏûÏ¢¿Õ¼ä
        }
      }
    }

    if (events & SBP_PERIODIC_EVT)                                 //½ÓÊÕµ½¡°°´Ò»¶¨ÖÜÆÚÖ´ÐÐÈÎÎñ¡±µÄÏûÏ¢
    {
      events &= ~SBP_PERIODIC_EVT;                                 //Îö¹¹ÏûÏ¢

      Util_startClock(&periodicClock);                             //¿ªÆôÖÜÆÚÖ´ÐÐÈÎÎñµÄÊ±ÖÓ

      // Perform periodic application task
      SimpleBLEPeripheral_performPeriodicTask();                   //Ö´ÐÐÖÜÆÚÐÔÈÎÎñ

      // led Á÷Ë®µÆ  
      if(TEST_LED_MODE_ALL_BLINK == my_led_mode)                   //ËùÓÐµÄledµÆ¶¼ÉÁË¸
      {
        static int count = 0;
      ...
      else if(TEST_LED_MODE_ALL_FLOW == my_led_mode)             //²âÊÔÁ÷Ë®µÆ
      {
        static uint8 i = 0;
        static int count = 0;
      ...
      if(0)//µç³Ø²âÊÔ
      {
          ...
          LCD_WRITE_STRING_VALUE("read: ", percent, 10, LCD_PAGE6);
          ...
          LCD_WRITE_STRING_VALUE("Battery: ", percent, 10, LCD_PAGE7);
      }
...
//      LCD_WRITE_STRING_VALUE("adc:", OneShotADC_value, 10, LCD_PAGE4);
    ...
    if (events & SBP_BUZZER_EVT)      //·äÃùÆ÷²âÊÔÏûÏ¢£¬ÔÚ´¥·¢·äÃùÆ÷Ê±£¬´ËÏûÏ¢¼ÓÔØµ½ÏûÏ¢¶ÓÁÐÖÐ
    {
      events &= ~SBP_BUZZER_EVT;      //Îö¹¹ÏûÏ¢
      board_pwm_stop();               //°åÔØpwmÍ£Ö¹
...
#ifdef FEATURE_OAD                    //Èç¹û¶¨ÒåOADÌØÐÔ  ÎÞÏßÎÞ·ì¹Ì¼þÉý¼¶ÌØÐÔ
...
static void SimpleBLEPeripheral_handleKeys(uint8_t shift, uint8_t keys)
{
  ...
  //LCD_WRITE_STRING_VALUE("key: 0x", keys, 16, LCD_PAGE7);
   if (keys & (KEY_LEFT | KEY_UP | KEY_RIGHT | KEY_DOWN | KEY_SELECT))   //ÏàÓ¦µÄ°´¼üËù´¦ÀíµÄÊÂÇé
   {
    static uint8 du = 0;
    du += 5;
    du %=100;
    ...
    board_pwm_start(4096, 50);  // freq:4096, ³ÖÐøÊ±¼ä£º50ms
    Util_startClock(&BuzzerClock);

    LCD_WRITE_STRING("Twsa     \r\n", LCD_PAGE7);
 ...
 * @return  None.
 */
static void SimpleBLEPeripheral_processAppMsg(sbpEvt_t *pMsg)        //Ö´ÐÐÓ¦ÓÃÏûÏ¢
{
  switch (pMsg->hdr.event)
  {
    case SBP_STATE_CHANGE_EVT:        //×´Ì¬¸Ä±äÊÂ¼þÏûÏ¢
      SimpleBLEPeripheral_processStateChangeEvt((gaprole_States_t)pMsg->
                                                hdr.state);
      break;

    case SBC_KEY_CHANGE_EVT:          //°´¼ü·­×ªÏûÏ¢  ÊÂ¼þ
      SimpleBLEPeripheral_handleKeys(0, pMsg->hdr.state); 
      break;      

    case SBP_CHAR_CHANGE_EVT:         //×Ö·ûÏûÏ¢ÊÂ¼þ
      SimpleBLEPeripheral_processCharValueChangeEvt(pMsg->hdr.state);
      break;
      
    case SBP_UART_CHANGE_EVT:// uart¸Ä±äÊÂ¼þ
    {        
        uint8_t valueToCopy[SIMPLEPROFILE_CHAR6_LEN] = {0};
        uint8 len = qq_read(valueToCopy, SIMPLEPROFILE_CHAR6_LEN);
        // ·¢ËÍ´®¿ÚÊý¾Ýµ½Ö÷Éè±¸£¬ÕâÀïÊÇ´®¿ÚÍ¸´«µÄÊý¾Ý£¬×î´óÒ»¸ö°üÎª  SIMPLEPROFILE_CHAR6_LEN
        if(len > 0)
        {   // Í¨¹ý SIMPLEPROFILE_CHAR6 ·¢ËÍÊý¾Ýµ½Ö÷Éè±¸
            SimpleProfile_SetParameter(SIMPLEPROFILE_CHAR6, len, valueToCopy);

//            LCD_WRITE_STRING_VALUE(" S ", len, 10, LCD_PAGE5);
        }
        // Èç¹û»¹ÓÐÊý¾Ý°ü£¬ÄÇÃ´¾Í·¢ËÍÒ»¸öÍ¨ÖªÏûÏ¢ SBP_UART_CHANGE_EVT
        if(qq_total() > 0)
        {   
            SimpleBLEPeripheral_enqueueMsg(SBP_UART_CHANGE_EVT, NULL);
        }
...
static void SimpleBLEPeripheral_charValueChangeCB(uint8_t paramID)
{
  SimpleBLEPeripheral_enqueueMsg(SBP_CHAR_CHANGE_EVT, paramID);      //×Ö·ûÏûÏ¢ÈëÁÐ
}
#endif //!FEATURE_OAD
...
static void SimpleBLEPeripheral_processCharValueChangeEvt(uint8_t paramID)
{
  ...
  switch(paramID)
  {
    ...
    case SIMPLEPROFILE_CHAR6: // ¶Á³öchar6ÖÐµÄÊý¾Ý£¬²¢·¢ËÍµ½´®¿Ú
...
void SimpleBLEPeripheral_keyChangeHandler(uint8 keys)
{
  SimpleBLEPeripheral_enqueueMsg(SBC_KEY_CHANGE_EVT, keys);  //°´¼üÏûÏ¢ÈëÁÐ
}
